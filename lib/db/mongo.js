...
mongoDB.prototype.open = function () {
  ...
  return mongoose.connect(this.connectionString, { useMongoClient: true });
};
...
mongoDB.prototype.getTipHash = function() {
  return Meta.getTipHash();
};

mongoDB.prototype.setTipHash = function(hash, cb) {
  return Meta.setTipHash(hash);
};

mongoDB.prototype.setChainOptions = function setChainOptions(options)  {
  return Meta.setChainOptions(options);
};

mongoDB.prototype.getChainOptions = function getChainOptions()  {
  return Meta.getChainOptions();
};

mongoDB.prototype.saveEntry = function saveEntry(hash, height, entry) {
...
mongoDB.prototype.getEntryByHash = function getEntryByHash(hash) {
  return Entry.getEntryByHash(hash);
};

mongoDB.prototype.getEntryByHeight = function getEntryByHeight(height) {
  return Entry.getEntryByHeight(height);
};

mongoDB.prototype.getBlockHeightByHash = function getBlockHeightByHash(hash) {
  return Block.getBlockHeightByHash(hash);
};

mongoDB.prototype.getBlockHashByHeight = function getBlockHashByHeight(height) {
...
mongoDB.prototype.saveTip = function saveTip(hash, height) {
  return Tip.saveTip(hash, height);
};

mongoDB.prototype.removeTip = function removeTip(hash) {
  return Tip.removeTip(hash);
};

mongoDB.prototype.getTips = function getTips()  {
  return Tip.getTips();
};
...
mongoDB.prototype.saveCoins = function saveCoins(key, data, coin) {
  return Coin.saveCoins(key, data, coin);
};
...
mongoDB.prototype.reset = async function reset() {
  await Block.remove({});
  await Tx.remove({});
  await Coin.remove({});
  await Entry.remove({});
