...
ChainDB.prototype._save = async function _save(entry, block, view) {
  ...
  const saveEntryEnd = util.hrtime(saveEntryStart);
  console.log(`Save Entry took: ${saveEntryEnd}`);
  ...
  const setTipHashStart = util.hrtime();
  await this.mongo.setTipHash(this.pending.commit(hash));
  console.log(`set tip hash took ${setTipHashStart}`);
  console.log('');
};
...
ChainDB.prototype.saveView = async function saveView(view) {
  ...
  for (const [hash, coins] of view.map) {
    for (const [index, coin] of coins.outputs) {
      console.log(coin);
      if (coin.spent) {
        // const removeStart = util.hrtime();
        await this.mongo.removeCoins(layout.c(hash, index));
        // console.log(`\t\t\t\t remove coins took: ${removeStart}`);
      ...
      // const saveStart = util.hrtime();
      await this.mongo.saveCoins(layout.c(hash, index), raw, coin);
      // console.log(`\t\t\t saveCoins took: ${saveStart}`);
      this.coinCache.push(hash + index, raw);
    }
  }

  const saveTestStart = util.hrtime();

  await this.mongo.saveTestCoins('somehash', coinArr);

  console.log(`\t\t\t saveTestStart took: ${saveTestStart}`);
};
...
ChainDB.prototype.connectBlock = async function connectBlock(entry, block, view) {
  ...
  if (!view.undo.isEmpty()) {
    const saveUndoStart = util.hrtime();
    await this.mongo.saveUndoCoins(layout.u(hash), view.undo.commit());
    console.log(`\t\t saveUndo took ${util.hrtime(saveUndoStart)}`);
  }
  ...
  const saveRemoveTipStart = util.hrtime();

  const removeTipStart = util.hrtime();
  await this.mongo.removeTip(entry.prevBlock);
  console.log(`\t\t removeTip took ${util.hrtime(removeTipStart)}`);

  const saveTipStart = util.hrtime();
  await this.mongo.saveTip(hash);
  console.log(`\t\t saveTip took ${util.hrtime(saveTipStart)}`);
};
