...
ChainDB.prototype.open = async function open() {
  ...
  if (state) {
    if (!this.options.spv) {
      await this.mongo.preflight(state);
    }
...
ChainDB.prototype.put = function put(dbOp) {
  assert(this.current);
...
ChainDB.prototype.commit = async function commit() {
  ...
  try {
    const op = util.hrtime();
    await this.current.write();
    console.log(`test took: ${util.hrtime(op)}`);
  } catch (e) {
    this.current = null;
    this.pending = null;
    this.coinCache.drop();
...
ChainDB.prototype.save = async function save(entry, block, view) {
  ...
  const commitStart = util.hrtime();
  await this.commit();
  console.log(`commit took: ${util.hrtime(commitStart)}`);
};
...
ChainDB.prototype._save = async function _save(entry, block, view) {
  ...
  const saveEntryStart = util.hrtime();

  // Entry data.
  this.put(() => {
    return this.mongo.saveEntry(hash, entry.height, entry.toRaw());
  });
  ...
  if (!entry.isGenesis()) {
    this.put(() => {
      return this.mongo.updateNextBlock(entry.prevBlock, hash);
    });
  }
  ...
  const saveBlockStart = util.hrtime();

  // Connect block and save data.
  await this.saveBlock(entry, block, view);
  console.log(`saveBlock took: ${util.hrtime(saveBlockStart)}`);
  // Commit new chain state.
  this.put(() => {
    return this.mongo.setTipHash(this.pending.commit(hash));
  });
};
...
ChainDB.prototype._disconnect = async function _disconnect(entry, block) {
  ...
  this.put(() => {
    return this.mongo.setTipHash(this.pending.commit(entry.prevBlock));
  });
...
ChainDB.prototype.saveUpdates = function saveUpdates() {
  ...
  for (const update of updates) {
    ...
    this.put(() => {
      return this.mongo.saveStateCache(layout.v(bit, hash), update.toRaw());
    });
  }
};
...
ChainDB.prototype.reset = async function reset(block) {
  ...
  for (;;) {
    ...
    if (tip.hash === entry.hash) {
      this.put(() => {
        return this.mongo.setTipHash(this.pending.commit(tip.hash));
      });
    ...
    this.put(() => {
      return this.mongo.removeTip(tip.hash);
    });
    this.put(() => {
      return this.mongo.saveTip(tip.prevBlock);
    });
    this.put(() => {
      return this.mongo.deleteEntry(tip.hash);
    });
    ...
    // Revert chain state to previous tip.
    this.put(() => {
      return this.mongo.setTipHash(this.pending.commit(tip.prevBlock));
    });
...
ChainDB.prototype._removeChain = async function _removeChain(hash) {
  ...
  for (;;) {
    ...
    this.put(() => {
      return this.mongo.deleteBcoinBlock(tip.hash);
    });
...
ChainDB.prototype.saveBlock = async function saveBlock(entry, block, view) {
  ...
  this.put(() => {
    return this.mongo.saveBcoinBlock(entry, block);
  });

  if (!view)
    return;
  const connectBlockStart = util.hrtime();
  await this.connectBlock(entry, block, view);
  console.log(`\t connectBlock took: ${util.hrtime(connectBlockStart)}`);
};
...
ChainDB.prototype.removeBlock = async function removeBlock(entry) {
  ...
  this.put(() => {
    return this.mongo.deleteBcoinBlock(block.hash());
  });
...
ChainDB.prototype.saveView = async function saveView(view) {
  for (const [hash, coins] of view.map) {
    for (const [index, coin] of coins.outputs) {
      if (coin.spent) {
        this.put(() => {
          return this.mongo.removeCoins(layout.c(hash, index));
        });
      ...
      this.put(() => {
        return this.mongo.saveCoins(layout.c(hash, index), raw, coin);
      });
...
ChainDB.prototype.connectBlock = async function connectBlock(entry, block, view) {
  ...
  for (let i = 0; i < block.txs.length; i++) {
    ...
    const indexTXStart = util.hrtime();
    this.indexTX(tx, view, entry, i);
    console.log(`\t\t indexTX took ${util.hrtime(indexTXStart)}`);
  }

  // Commit new coin state.
  const saveViewStart = util.hrtime();
  await this.saveView(view);
  console.log(`\t\t saveView took ${util.hrtime(saveViewStart)}`);

  // Write undo coins (if there are any).
  if (!view.undo.isEmpty()) {
    this.put(() => {
      return this.mongo.saveUndoCoins(layout.u(hash), view.undo.commit());
    });
  }
  ...
  this.put(() => {
    return this.mongo.removeTip(entry.prevBlock);
  });
  this.put(() => {
    return this.mongo.saveTip(hash);
  });
};
...
ChainDB.prototype.disconnectBlock = async function disconnectBlock(entry, block) {
  ...
  // Remove undo coins.
  this.put(() => {
    return this.mongo.removeUndoCoins(layout.u(hash));
  });
...
ChainDB.prototype.pruneBlock = async function pruneBlock(entry) {
  ...
  this.put(() => {
    return this.mongo.deleteBcoinBlock(hash);
  });
...
ChainDB.prototype.indexTX = async function indexTX(tx, view, entry, index) {
  ...
  if (this.options.indexTX) {
    ...
    this.put(() => {
      return this.mongo.saveBcoinTx(entry, tx, meta);
    });
  }
  ...
  if (!tx.isCoinbase()) {
    for (const {prevout} of tx.inputs) {
      ...
      if (!addr)
        continue;
      this.put(() => {
        return this.mongo.removeAddress(layout.C(addr, prevout.hash, prevout.index));
      });
    }
  }
  ...
  for (let i = 0; i < tx.outputs.length; i++) {
    ...
    this.put(() => {
      return this.mongo.saveAddress(layout.C(addr, hash, i), addr, hash, i);
    });
  }
};
...
ChainDB.prototype.unindexTX = async function unindexTX(tx, view) {
  ...
  if (this.options.indexTX) {
    this.put(() => {
      return this.mongo.deleteBcoinTx(hash);
    });
  }
  ...
  if (!tx.isCoinbase()) {
    for (const { prevout } of tx.inputs) {
      ...
      this.put(() => {
        return this.mongo.saveAddress(layout.C(addr, prevout.hash, prevout.index));
      });
    }
  }
  ...
  for (let i = 0; i < tx.outputs.length; i++) {
    ...
    this.put(() => {
      return this.mongo.removeAddress(layout.C(addr, hash, i));
    });
  }
};
