...
ChainDB.prototype.getHash = async function getHash(height) {
  ...
  const hash = await this.mongo.getBlockHashByHeight(height);
...
ChainDB.prototype.getEntryByHeight = async function getEntryByHeight(height) {
  ...
  const data = await this.mongo.getBlockHashByHeight(height);
...
ChainDB.prototype.prune = async function prune(tip) {
  ...
  for (let i = start; i <= end; i++) {
    ...
    await this.mongo.deleteBcoinBlock(hash);
...
/**
 * Check whether coins are still unspent below a height.
 * @see https://bitcointalk.org/index.php?topic=67738.0
 * @param {TX} tx
 * @param {height} height
 * @returns {Promise} - Returns Boolean.
...
ChainDB.prototype.hasDupeCoins = async function hasDupeCoins(tx, height) {
  for (let i = 0; i < tx.outputs.length; i++) {
    const key = layout.c(tx.hash(), i);

    if (await this.mongo.hasDupeCoins(key, height))
      return true;
  }
  return false;
};
...
ChainDB.prototype._save = async function _save(entry, block, view) {
  ...
  if (!entry.isGenesis())
    await this.mongo.updateNextBlock(entry.prevBlock, hash);
...
ChainDB.prototype.saveUpdates = async function saveUpdates() {
  ...
  for (const update of updates) {
    ...
    await this.mongo.saveStateCache(layout.v(bit, hash), update.toRaw());
  }
};
...
ChainDB.prototype.reset = async function reset(block) {
  ...
  for (;;) {
    ...
    await this.mongo.deleteEntry(tip.hash);
...
ChainDB.prototype._removeChain = async function _removeChain(hash) {
  ...
  for (;;) {
    ...
    await this.mongo.removeTip(tip.hash);
    await this.mongo.deleteEntry(tip.hash);
    await this.mongo.deleteBcoinBlock(tip.hash);
