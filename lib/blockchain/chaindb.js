...
ChainDB.prototype.open = async function open() {
  ...
  let walletAddrs;
  try {
    walletAddrs = await this.mongo.getWalletAddresses();
  } catch (e) {
    this.logger.error(e);
  }

  if (walletAddrs) {
    walletAddrs.forEach((addr) => {
      const cache = this.walletCache[addr.address];
      if (cache) {
        this.walletCache[addr.address].push(addr.wallet);
      } else {
        this.walletCache[addr.address] = [addr.wallet];
      }
    });
  }
...
ChainDB.prototype.saveView = async function saveView(view) {
  for (const [hash, coins] of view.map) {
    for (const [index, coin] of coins.outputs) {
      ...
      const addr = coin.output.toJSON().address || '';
      const wallet = this.walletCache[addr] || [];
      await this.mongo.saveCoins(layout.c(hash, index), raw, coin, hash, index, wallet);
