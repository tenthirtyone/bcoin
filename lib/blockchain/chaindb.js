...
ChainDB.prototype.prune = async function prune(tip) {
  ...
  for (let i = start; i <= end; i++) {
    ...
    this.mongo.removeUndoCoins(layout.u(hash));
  }
...
ChainDB.prototype.saveView = function saveView(view) {
  for (const [hash, coins] of view.map) {
    for (const [index, coin] of coins.outputs) {
      if (coin.spent) {
        this.mongo.removeCoins(layout.c(hash, index));
      ...
      this.mongo.saveCoins(layout.c(hash, index), raw, coin);
...
ChainDB.prototype.connectBlock = async function connectBlock(entry, block, view) {
  ...
  if (!view.undo.isEmpty()) {
    this.mongo.saveUndoCoins(layout.u(hash), view.undo.commit());
  }
...
ChainDB.prototype.indexTX = async function indexTX(tx, view, entry, index) {
  ...
  if (!tx.isCoinbase()) {
    for (const {prevout} of tx.inputs) {
      ...
      this.mongo.removeAddress(layout.C(addr, prevout.hash, prevout.index));
    }
  }
